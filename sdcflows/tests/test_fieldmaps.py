...
def test_type_setter():
    ...
    assert fm._type_setter(obj, 'method', fm.EstimatorType.PEPOLAR) == fm.EstimatorType.PEPOLAR
...
def test_FieldmapEstimation_missing_files(tmpdir, dsA_dir):
    ...
    with pytest.raises(ValueError, match=r'.* \(phase1/2\) .* \(magnitude2 file\) is missing.*'):
        fm.FieldmapEstimation(
            [
                fm.FieldmapFile('data/sub-01_phase1.nii.gz', metadata={'EchoTime': 0.004}),
                fm.FieldmapFile('data/sub-01_phase2.nii.gz', metadata={'EchoTime': 0.007}),
            ]
        )
...
def test_FieldmapFile_filename(tmp_path, dsA_dir):
    ...
    fm.FieldmapEstimation(fm.FieldmapFile(datadir / 'sub-01_phasediff.nii.gz'))
    fm.clear_registry()
